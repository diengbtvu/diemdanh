...
from models import ResNet50Face
...
api = Api(
    app,
    version='1.0',
    title='Face Recognition API - ResNet50',
    description='API nhận dạng khuôn mặt sử dụng ResNet50 (Transfer Learning) và MTCNN',
    doc='/swagger/',
    prefix='/api/v1'
)
...
try:
    CLASS_NAMES = [f"student_{i:03d}" for i in range(294)]  # 294 classes
    print(f"[INFO] Loaded {len(CLASS_NAMES)} classes")
except Exception as e:
    print(f"[WARNING] Could not load class names: {e}")
    CLASS_NAMES = [f"class_{i}" for i in range(294)]
    ...
    try:
        ...
        if boxes is not None:
            ...
            for i, box in enumerate(boxes):
                ...
                print(f"[DEBUG] Running ResNet50 prediction for face {i+1}")
                with torch.no_grad():
...
def home():
    """Home endpoint"""
    return jsonify({
        "message": "Face Recognition API - ResNet50",
        "model": "ResNet50 (Transfer Learning) - 100% Test Accuracy",
...
if __name__ == '__main__':
    print(f"[INFO] ====================================")
    print(f"[INFO] ResNet50 Face Recognition API")
    print(f"[INFO] ====================================")
    print(f"[INFO] Device: {DEVICE}")
    print(f"[INFO] Model: ResNet50 (Test Acc: 100%)")
    print(f"[INFO] Classes: {len(CLASS_NAMES)}")
    print(f"[INFO] Image Size: {IMG_SIZE}x{IMG_SIZE}")
    print("[INFO] Available endpoints:")
    print("  - Swagger: http://localhost:5002/swagger/")
